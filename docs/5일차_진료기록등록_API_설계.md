...
| 접근 권한 | 사내 **의료인**(MEDICAL 부서, PENDING 제외) 전용 — `_require_medical_staff` 의존성 적용 |
...
ERD 기준으로 진료기록과 X-Ray 이미지는 **별도 테이블로 분리**되어 있으며, 등록 시 두 테이블에 하나의 트랜잭션으로 INSERT한다.
...
### 3.2 xray_images (X-Ray 이미지) — 신규 모델

| 필드 | 타입 | 제약 | 설명 |
|---|---|---|---|
| `id` | bigint | PK, auto increment | 이미지 ID |
| `record_id` | bigint | FK → medical_records.id, `ondelete=CASCADE`, NOT NULL | 진료 기록 참조 |
| `file_path` | varchar(255) | NOT NULL | 로컬 저장소 이미지 파일 경로 |
| `created_at` | datetime | NOT NULL, default: current_timestamp | 등록 일시 |

> **삭제 정책 (REQ-PTNT-005 연계)**: 환자 삭제 → 진료기록 → X-Ray 이미지 순으로 DB 레벨 CASCADE로 연쇄 삭제된다. 파일 시스템의 이미지 파일은 환자 삭제 담당(C)의 로직에서 `media/` 경로 기준으로 삭제한다.
...
| 필드 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `chart_number` | str | ✅ | 진료 차트 넘버 (중복 불가) |
| `symptoms` | str | ✅ | 진료된 증상 |
| `xray_image` | file | ✅ | 촬영된 흉부 X-Ray 이미지 (jpg/png) |

#### 처리 흐름

1. 환자 존재 확인 (`get_patient` 재사용) — 없으면 404
2. 차트 넘버 중복 검사 (UNIQUE 제약) — 중복이면 409
3. 이미지 형식 검증 (jpg/png) — 아니면 422
4. 이미지를 `media/xray/` 에 저장 (파일명은 uuid로 충돌 방지)
5. `medical_records` + `xray_images` 두 테이블 INSERT 후 commit
   (실패 시 rollback 및 저장된 파일 삭제)
...
```json
{
  ...
  "xray_image": {
    "id": 3,
    "file_path": "media/xray/1_a1b2c3d4.png"
  }
}
```
...
| 409 | 이미 존재하는 차트 넘버 |
| 422 | 필수 항목 누락 또는 이미지 형식 오류 (jpg/png 외) |
...
| 요구사항 | 반영 방안 |
|---|---|
| 모든 API 3초 이내 응답 | 비동기(async) 처리로 이미지 수신·저장, 로컬 파일 시스템 저장으로 네트워크 지연 제거, 트랜잭션 내 단순 INSERT 2건으로 로직 최소화 |
